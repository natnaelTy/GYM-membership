  ...
  try {
    ...
    pool.query(checkIfUserExist, [phone_number], async (error, result) => {
      ...
      if (result.length === 0) {
        ...
        pool.query(query, [full_name, phone_number, hashedPassword, gender, image], async (error, result) => {
          if (error){
...
export const login = (req, res) => {
  ...
  try{
    ...
    if(result.length === 0){
      return res.status(400).json({success: false, message: "Incorrect phone number or password!"});
    }
    ...
    const token = generateTokenAndSetCookie(res, user.userId, user.full_name, phone_number);
    // if this all valid login and return the user
    return res.status(200).json({success: true, message: "You are successfully logged in", user: {...user, user_password: undefined}, token });
    });
  }catch(err){
